document.addEventListener('DOMContentLoaded', function() {
    ...
    const MODEL_OPTIONS = [
      // DeepInfra Models
      { value: 'google/gemini-2.0-flash-001', label: 'Gemini 2.0 Flash (DeepInfra)', provider: 'deepinfra' },
      { value: 'anthropic/claude-3-7-sonnet-latest', label: 'Claude 3.7 Sonnet (DeepInfra)', provider: 'deepinfra' },
      { value: 'anthropic/claude-4-sonnet', label: 'Claude 4 Sonnet (DeepInfra)', provider: 'deepinfra' },
      { value: 'mistralai/Mistral-Small-3.1-24B-Instruct-2503', label: 'Mistral Small 3.1 24B (DeepInfra)', provider: 'deepinfra' },
      { value: 'mistralai/Devstral-Small-2505', label: 'Devstral Small 2505 (DeepInfra)', provider: 'deepinfra' },
      { value: 'mistralai/Mistral-Small-24B-Instruct-2501', label: 'Mistral Small 24B (DeepInfra)', provider: 'deepinfra' },
      { value: 'meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8', label: 'Llama 4 Maverick 17B (DeepInfra)', provider: 'deepinfra' },
      { value: 'deepseek-ai/DeepSeek-R1-0528', label: 'DeepSeek R1 0528 (DeepInfra)', provider: 'deepinfra' },
      { value: 'deepseek-ai/DeepSeek-R1-Turbo', label: 'DeepSeek R1 Turbo (DeepInfra)', provider: 'deepinfra' },
      ...
      { value: 'anthropic/claude-3-5-sonnet', label: 'Claude 3.5 Sonnet (OpenRouter)', provider: 'openrouter' },
      { value: 'anthropic/claude-3-5-haiku', label: 'Claude 3.5 Haiku (OpenRouter)', provider: 'openrouter' },
      { value: 'google/gemini-pro-1.5', label: 'Gemini Pro 1.5 (OpenRouter)', provider: 'openrouter' },
      { value: 'meta-llama/llama-3.1-8b-instruct', label: 'Llama 3.1 8B (OpenRouter)', provider: 'openrouter' },
      { value: 'meta-llama/llama-3.1-70b-instruct', label: 'Llama 3.1 70B (OpenRouter)', provider: 'openrouter' },
      { value: 'mistralai/mistral-7b-instruct', label: 'Mistral 7B (OpenRouter)', provider: 'openrouter' },
      { value: 'deepseek/deepseek-chat', label: 'DeepSeek Chat (OpenRouter)', provider: 'openrouter' }
    ];
            ...
            <div class="form-group">
                ...
                <select id="action-model">
                    <optgroup label="DeepInfra Models">
                        ${MODEL_OPTIONS.filter(opt => opt.provider === 'deepinfra').map(opt => `<option value="${opt.value}" ${action.model === opt.value ? 'selected' : ''}>${opt.label}</option>`).join('')}
                    </optgroup>
                    <optgroup label="OpenRouter Models">
                        ${MODEL_OPTIONS.filter(opt => opt.provider === 'openrouter').map(opt => `<option value="${opt.value}" ${action.model === opt.value ? 'selected' : ''}>${opt.label}</option>`).join('')}
                    </optgroup>
                </select>
            </div>
    ...
    if (openrouterApiKeyInput) {
        ...
        openrouterApiKeyInput.addEventListener('input', debounce(async function() {
            const key = openrouterApiKeyInput.value.trim();
            openrouterApiKeyInput.style.transition = 'border-color 0.3s';
            openrouterApiKeyInput.style.borderColor = '#fdba74'; // Indicate checking

            if (!key) {
                openrouterApiKeyInput.style.borderColor = '#e5e7eb'; // Reset to default
            ...
            try {
                ...
                openrouterApiKeyInput.style.borderColor = '#4ade80'; // Valid key
            } catch (err) {
                chrome.storage.sync.remove('openrouterApiKey');
                openrouterApiKeyInput.style.borderColor = '#f87171'; // Invalid key
            }
        }, 500));
    ...
    // --- Initialization ---
    function initialize() {
        chrome.storage.sync.get(['actionButtonsSettings', 'isMicroButtonVisible', 'isReadButtonVisible', 'deepinfraApiKey', 'openrouterApiKey'], (result) => {
