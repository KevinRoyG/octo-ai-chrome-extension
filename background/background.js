...
// Fonction utilitaire pour obtenir la clé API DeepInfra
...
async function handleGrammarCorrection(text, tabId) {
  try {
    ...
    console.log('[SpeedReader] Prompt envoyé à DeepInfra:', prompt);
    // Récupérer la clé API utilisateur
    const apiKey = await getDeepInfraApiKey();
    if (!apiKey) throw new Error('Aucune clé API DeepInfra trouvée. Veuillez la renseigner dans le popup.');
    ...
    console.log('[SpeedReader] HTTP status:', response.status);
    const data = await response.json();
    console.log('[SpeedReader] Réponse brute DeepInfra:', data);
    let corrected = "";
    if (
      data &&
      data.choices &&
...
async function handleCustomAction(request, tabId) {
  try {
    ...
    console.log('[SpeedReader] HTTP status:', response.status);
    const data = await response.json();
    console.log('[SpeedReader] Raw DeepInfra response:', data);
    let resultText = "";
    if (
      data &&
      data.choices &&
